<?php
    ...
    function curl_get_qianmi_token($refresh_token){
		$refresh_token='ee89cc0e4652428c9f1fa3022065da38';
		...
		foreach($post_data as $x=>$x_value)
		  ...
		  echo $totalStr."<br>";
		  $sign= sha1($appSecret.$totalStr.$appSecret);
		$post_data [ 'sign' ] =  $sign;
		 echo $sign."<br>"; 
		 foreach($post_data as $x=>$x_value)
		  {
        ...
        file_put_contents(qianmi_token_file(),json_encode($TOKEN_json));//保存到本地
        return $TOKEN_json;
    }
    ...
    function file_get_qianmi_token($now_time){
        ...
        //if( !is_array($token_array) || !isset($token_array['refresh_token']) ){
            //去千米获取，然后保存
 ...
 curl_get_qianmi_token('');
    $now_time = time();
    //$token_array = file_get_qianmi_token($now_time);
